...
- `cloudinit_datasource` (String)  
  ...
  Options:  
    - `nocloud`  
    - `config_drive_v2`
- `cloudinit_files` (List)  
  A list of objects representing the cloud-init files. Each object has:
  - `name` (String) - The name of the file (e.g. `"user-data"`, `"meta-data"`, `"vendor-data"`, `"network-data"`).
  - `contents` (String) - The file content, typically YAML for user-data and JSON for meta-data with `config_drive_v2`.
